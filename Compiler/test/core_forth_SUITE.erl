...
compile_clause() ->
    ...
    io:format(standard_error, "~s~n",
              [e4c_pass_kern:format_core_forth(lists:flatten(Compiled), [])]).

compile_helper(RootScope, Code) ->
    Scope = #e4scope{vars=RootScope},
  S0 = e4c_pass_kern:module_new(),
    S1 = lists:foldl(fun(Var, St) -> e4c_pass_kern:stack_push(St, Var) end,
                     S0, RootScope),
  S2 = e4c_pass_kern:scope_push(S1, Scope),
  S = e4c_pass_kern:process_code(S2, Code),
    lists:reverse(e4c_pass_kern:get_code(S)).
