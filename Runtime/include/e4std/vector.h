...
class Vector {
 ...
 public:
  ...
  explicit Vector(::size_t init_size) { resize(init_size); }
...
class PODVector : public impl::VectorImpl {
 public:
  ...
  explicit PODVector(::size_t init_size) { resize(init_size); }

  PODVector(PODVector&& other) = default;

  PODVector& operator=(PODVector&& other) = default;

  ValueType* data() { return reinterpret_cast<ValueType*>(data_.get()); }
  ...
  ValueType* binary_search(const ValueType* val, VoidpCompareFun cmp) const {
    return static_cast<ValueType*>(::bsearch(
      static_cast<const void*>(val), static_cast<const void*>(data_.get()),
      size(), sizeof(ValueType), cmp));
  }

  void sort(VoidpCompareFun cmp) {
    ::qsort(static_cast<void*>(data_.get()), size(), sizeof(ValueType), cmp);
  }
