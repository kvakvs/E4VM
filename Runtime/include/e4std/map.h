...
struct MapNode {
  ...
  MapNode(const KeyType& k, const ValueType& v, MapNode* left, MapNode* right)
...
class Map {
 ...
 public:
  ...
  ::size_t size() const { return count_nodes_helper(root_); }

  ::size_t depth() const { return depth_helper(this->root_); }
  ...
  bool remove(const KeyType& key) { return remove_helper(nullptr, root_, key); }

  NodeType* find(const KeyType& key) const { return find_helper(root_, key); }

 private:
  void insert_helper(NodeType* root, const KeyType& key, const ValueType& val) {
  ...
  ::size_t depth_helper(const NodeType* root) const {
    if (not root) {
      ...
      return 1 + max(depth_helper(root->left_), depth_helper(root->right_));
    }
  }

  bool remove_helper(NodeType* parent, NodeType* current, const KeyType& key) {
    if (not current) {
      return false;
    }
